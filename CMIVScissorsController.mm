...
- (void)windowWillClose:(NSNotification *)notification
{
	...
	[crossAxialView setTranlateSlider:nil];
	[originalView setDrawing: NO];
	[cprView setDrawing: NO];
	[crossAxialView setDrawing: NO];
	if (!isInWizardMode)
                        ...
                        :(CMIV_CTA_TOOLS*) owner
{
	...
	// Initialize original view CPRView (bottom right) and Axial View (top right)
	err = [self initViews];
	if (err)
		return nil;
	...
	[crossAxialView setTranlateSlider:axImageSlider];
	[crossAxialView setHorizontalSlider:nil];
...
#ifdef VERBOSEMODE
	NSLog( @"************************* %@ 2D View established! ********************************", NSStringFromClass([self class]));
#endif
    
	// FIXME: mistery bug happens when DCMView.h is not updated, scaleValue will be 0, without following code.
...
//	
//}
- (id)showPanelAsCPROnly:(ViewerController *) vc
                        :(CMIV_CTA_TOOLS*) owner
{
	isInitialWithCPRMode=YES;
...
- (id) showPanelAfterROIChecking:(ViewerController *) vc
                                :(CMIV_CTA_TOOLS*) owner
{
	//int err=0;
	originalViewController=vc;	
...
- (void)changAmongMPRCPRAndAnalysis:(int)modeindex
{
	...
	if (modeindex==0)
	{
		...
		BOOL showcross = [[NSUserDefaults standardUserDefaults] boolForKey:@"CMIV2DViewShowAxViewCrossHair"];
		if (showcross==TRUE)
		{
			[axViewCrossShowButton setState:NSControlStateValueOn];
			[crossAxialView showCrossHair];
			[self updateAxView];
		}

        showcross = [[NSUserDefaults standardUserDefaults] boolForKey:@"CMIV2DViewShowCViewCrossHair"];
		if (showcross==TRUE)
		{
	...
	else if (modeindex==1)
	{
		...
		[crossAxialView setCrossCoordinates:-9999 :-9999 :YES];
		[crossAxialView hideCrossHair];
...
- (int) initViews
{
    ...
    vtkImageData *tempIm = oViewSlice->GetOutput();
...
//	[oViewROIList addObject:[NSMutableArray arrayWithCapacity:0]];
	...
	[crossAxialView setDCM:axViewPixList :fileList :axViewROIList :0 :'i' :YES];
	viewName = @"Original";
	[crossAxialView setStringID: viewName];
	[crossAxialView setMPRAngle: 0.0];
	[crossAxialView showCrossHair];
	
	[crossAxialView setIndexWithReset: 0 :YES];
    ...
    [crossAxialView setCrossCoordinates:crossX :crossY :YES];

	{
        // Cross axial view (top-right panel)
        BOOL showcross=[[NSUserDefaults standardUserDefaults] boolForKey:@"CMIV2DViewShowAxViewCrossHair"];
        if (showcross==FALSE)
		{
			showcross=TRUE;
			[[NSUserDefaults standardUserDefaults] setBool:showcross forKey:@"CMIV2DViewShowAxViewCrossHair"];
		}
        if (showcross==FALSE)  // TBC: never execute since refactoring ?
		{
			[crossAxialView setCrossCoordinates:-9999 :-9999 :YES];
			[crossAxialView hideCrossHair];
			[axViewCrossShowButton setState: NSControlStateValueOff];
        ...
        // CPR view (bottom-right panel)
        showcross = [[NSUserDefaults standardUserDefaults] boolForKey:@"CMIV2DViewShowCViewCrossHair"];
        if (showcross==FALSE)
		{
			showcross=TRUE;
			[[NSUserDefaults standardUserDefaults] setBool:showcross forKey:@"CMIV2DViewShowCViewCrossHair"];
		}
        if (showcross==FALSE) // TBC: never execute since refactoring ?
		{
			[cprView setCrossCoordinates:-9999 :-9999 :YES];
        ...
        // Original view (bottom-left panel)
        showcross = [[NSUserDefaults standardUserDefaults] boolForKey:@"CMIV2DViewShowOViewCrossHair"];
		if (showcross==FALSE)
		{
			showcross=TRUE;
			[[NSUserDefaults standardUserDefaults] setBool:showcross forKey:@"CMIV2DViewShowOViewCrossHair"];
		}
        if (showcross==FALSE) // TBC: never execute since refactoring ?
		{
...
{
#ifdef VERBOSEMODE
    NSLog(@"Original view update start");
#endif
...
//	}
	...
	if (currentViewMode==0)//if MPR mode
	{
		if (contrastVolumeData)
		{
            vtkImageData *tempROIIm = oViewROISlice->GetOutput();
			...
			[self creatROIListFromSlices:[oViewROIList objectAtIndex: 0]
                                        :imExtent[ 1]-imExtent[ 0]+1
                                        :imExtent[ 3]-imExtent[ 2]+1
                                        :imROI
                                        :oViewSpace[0]
	...
	tempIm->GetOrigin( oViewOrigin);	
#ifdef VERBOSEMODE
    NSLog(@"Original view update end");
#endif
}
...
- (void) updateAxView
{
    ...
    vtkImageData *tempIm = axViewSlice->GetOutput();
    tempIm->GetExtent( imExtent);
	tempIm->GetSpacing( axViewSpace);
	tempIm->GetOrigin( axViewOrigin);	
	...
	[axViewPixList addObject: mypix];
	[mypix release];
	float scale=[crossAxialView scaleValue];
	NSPoint newOrigin;
	...
	[crossAxialView setOrigin: newOrigin];
	...
	if (axViewROIMode==2) // use fuzzy connectedness segment results
	{
        ...
        vtkImageData *tempROIIm = axViewROISlice->GetOutput();
		...
		[self creatAxROIListFromFuzzyConnectedness:[axViewROIList objectAtIndex: 0]
                                                  :axROIExtent[ 1]-axROIExtent[ 0]+1
                                                  :axROIExtent[ 3]-axROIExtent[ 2]+1
                                                  :imAxROI
                                                  :axROISpacing[0]
                                                  :axROISpacing[1]
                                                  :axROIOrigin[0]
	...
	[crossAxialView setIndex: 0 ];
#ifdef VERBOSEMODE
    NSLog(@"Axial view update end");
#endif
}
...
- (void) changeCurrentTool:(ToolMode) tag
{
	...
	else if (tag==tNext)
	{
		...
		[crossAxialView setCurrentTool:tMeasure];
	...
	else if (tag==tROI)
	{
        ...
        if (currentTool==t3DRotate || currentTool==tMeasure)
		{
			currentTool=tag;
			[self updateOView];
			[self cAndAxViewReset];
	...
	else if (tag==t3DRotate)
	{
        ...
        if (currentTool==tROI || currentTool==tMeasure)
		{	
			currentTool=tag;
			[self updateOView];
			[self cAndAxViewReset];
	...
	else if (tag==tOval)
	{
		...
		[crossAxialView setCurrentTool: tRepulsor];
	}
	else
		return;
	...
	if ([sender isKindOfClass:[DCMPix class]]&&(!isChangingWWWLBySelf))
	{
            ...
            [axViewPixList containsObject: otherPix])
		{
			...
			if ([oViewPixList containsObject: otherPix])
			{
                ...
                if (iww != [crossAxialView curWW] || iwl != [crossAxialView curWL])
					[crossAxialView setWLWW:iwl :iww];
			}
	...
	else if (activeView==cprView)
	{
		if ([[[note userInfo] objectForKey:@"action"] isEqualToString:@"dragged"])
		{
			...
			if ([cprView angle]==0)
			{
				...
				int imExtent[ 6];
				double space[ 3], origin[ 3];
                vtkImageData *tempIm = cViewSlice->GetOutput();
                tempIm->GetExtent( imExtent);
				tempIm->GetSpacing( space);
				tempIm->GetOrigin( origin);	
...
//			}
	...
	else if (activeView==crossAxialView)
	{
		if ([[[note userInfo] objectForKey:@"action"] isEqualToString:@"dragged"])
		{
			interpolationMode=0;
			if ([crossAxialView angle]==0)
			{
				if (interpolationMode)
					axViewSlice->SetInterpolationModeToCubic();
				else
				...
				[crossAxialView getCrossCoordinates: &oX  :&oZ];
				oZ = -oZ;
				oX = oX*space[0]+origin[0];
			...
			{
				float angle= [crossAxialView angle]-lastAxViewZAngle;
				oViewUserTransform->RotateY(-angle);;
				[self updateOView];
				[self updateCView];
				lastAxViewZAngle=[crossAxialView angle];
			}
		}
...
- (NSInteger)numberOfRowsInTableView:(NSTableView *)tableView
{
    ...
    if ([centerlinesList isEqual: tableView ])
	{
	...
	if ([seedsList isEqual:aTableView])
	{
		...
		if ([[aTableColumn identifier] isEqualToString:@"Name"])
		{
			if ([anObject length]>0)
			{
                ...
                NSString* oldname=[[contrastList objectAtIndex:rowIndex] objectForKey:@"Name"];
				ROI* temproi;
				for (unsigned i=0;i<[totalROIList count];i++)
				{
					temproi=[totalROIList objectAtIndex: i];
...
- (int) generateSlidingNormals:(int)npts
                              :(double*)pointsxyz
                              :(double*)ptnormals
{
...
- (int) generateUnitRobbin:(int)npts
                          :(double*)inputpointsxyz
                          :(double*)ptnormals
                          :(double*)outputpointsxyz
                          :(double)angle
                          :(double)width
{
	/*
...
- (float*) calculateStraightCPRImage: (int*)pwidth
                                    : (int*)pheight
{
	...
	// Create a narrow 3D ribbon from the centerline
    // and use this ribbon to get cross-section line from each pair of points along this ribbon
    ...
    float *im = (float*)malloc(sizeof(float)*width*height);
	if (!im)
		return 0L;
...
- (float*) calculateCurvedMPRImage :(int*)pwidth :(int*)pheight
{
	...
	int pointNumber;
	double position[3];
	// Calculate parameters for CPR image (width, height, translateLeftX-Z,translateRightX-Z)
	int width, height;
	float path2DLength;	
...
-(NSImage*) imageForQuickTime:(NSNumber*) cur
                     maxFrame:(NSNumber*) max
{
...
- (IBAction)showCPRImageDialog:(id)sender
{
	if (cViewMPRorCPRMode)
	{
		...
		NSRunAlertPanel(NSLocalizedString(@"no CPR image", nil),
                        NSLocalizedString(@"Please choose CPR tools again.", nil),
                        NSLocalizedString(@"OK", nil), nil, nil);
}
	...
	if (tag)
	{
        ...
        for (pathindex=startindex;pathindex<endindex;pathindex++)
		{
			...
			if ([ifExportCrossSectionButton state]== NSControlStateValueOn)
			{
				...
				{
					NSString* tempstr=@"Cross Section ";
					unsigned int row;
                                               ...
                                               :(int)slicenumber
{
	...
	distanceofstep = step;
	NSRect viewsize = [crossAxialView frame];
	maxwidth=viewsize.size.width/[crossAxialView scaleValue];
	maxheight=viewsize.size.height/[crossAxialView scaleValue];
	float* newVolumeData=nil;
	long size= sizeof(float)*maxwidth*maxheight*imageNumber;
	newVolumeData=(float*) malloc(size);
	if (!newVolumeData)
...
- (IBAction)exportSingleImageToBasket:(id)sender
{
	...
	switch(tag)
	{
        ...
        case 3:
			viewsize = [crossAxialView frame];
			imgwidth=viewsize.size.width/[crossAxialView scaleValue];
			imgheight=viewsize.size.height/[crossAxialView scaleValue];
			imgdata=(float*)malloc(imgwidth*imgheight*sizeof(float));
			if (imgdata)
			{
				newPix=[self getCurPixFromAxView:imgdata:imgwidth:imgheight:imgRoiList];
				newCellICONForBasket=[crossAxialView nsimage];
			}			
			break;
	}
...
- (DCMPix*)getCurPixFromCView:(float*)imgdata:(int)imgwidth:(int)imgheight:(NSMutableArray*)imgROIs
{
	...
	for(j=0;j<[[cViewROIList objectAtIndex: 0] count];j++)
	{
		...
		if (([temproi type] == tMeasure || [temproi type] == tClosedPolygon) && [temproi valid])
		{
			ROI* anewroi=[[ROI alloc] initWithType: tMeasure :cViewSpace[0]  :cViewSpace[1] : NSMakePoint( origin[0],  origin[1])];
...
- (DCMPix*)getCurPixFromAxView:(float*)imgdata :(int)imgwidth :(int)imgheight :(NSMutableArray*)imgROIs
{
	...
	point[0]=[crossAxialView ConvertFromUpLeftView2GL:point[0]];
	point[1]=[crossAxialView ConvertFromUpLeftView2GL:point[1]];
	point[2]=[crossAxialView ConvertFromUpLeftView2GL:point[2]];
	point[3]=[crossAxialView ConvertFromUpLeftView2GL:point[3]];
...
{
    ToolMode seedingtoolindex = (ToolMode)(2-[seedingToolMatrix selectedColumn] + 6);
...
- (IBAction)addSeed:(id)sender
{
	...
	[contrast setObject:[seedName stringValue]  forKey:@"Name"];
...
- (int) initSeedsList
{
	...
	if (!isInWizardMode)
	{
		...
		contrast = [NSMutableDictionary dictionary];
		[contrast setObject:@"barrier"  forKey:@"Name"];
	...
	{		
		...
		[contrast setObject: [NSNumber numberWithFloat:3.0] forKey:@"BrushWidth"];
		[contrast setObject: [NSNumber numberWithInt:8] forKey:@"CurrentTool"];
		[contrast setObject:@"Automatic Procedure" forKey:@"Tips"];
		[contrastList addObject: contrast];
		...
		contrast = [NSMutableDictionary dictionary];
		[contrast setObject:@"Aorta" forKey:@"Name"];
		[contrast setObject: [NSColor greenColor] forKey:@"Color"];
		[contrast setObject: [NSNumber numberWithInt:7] forKey:@"CurrentTool"];
		[contrast setObject: [NSNumber numberWithFloat:2.0] forKey:@"BrushWidth"];
		[contrast setObject:@"Automatic Procedure" forKey:@"Tips"];
...
- (void) roiChanged: (NSNotification*) note
{
	...
	else if (currentTool==9)
	{
		...
		else if (roi==axViewMeasurePolygon && [crossAxialView mouseDragging])
		{
	...
	else if (cViewMPRorCPRMode && curvedMPR2DPath &&[[oViewROIList objectAtIndex: 0] containsObject: sender])
	{
		...
		if (roitype==tOpenPolygon && currentPathMode==ROI_drawing)
		{
...
-(void) dcmViewMouseUp: (NSNotification*) note
{
	if (currentTool==tOval)
	{
		...
		else if (sender==crossAxialView&&axViewMeasureNeedToUpdate)
		{
...
- (void)initCenterList
{
	...
	DCMPix * curImage= [cViewPixList objectAtIndex:0];
	curvedMPRReferenceLineOfAxis=[[ROI alloc] initWithType: tMeasure :[curImage pixelSpacingX] :[curImage pixelSpacingY] : NSMakePoint( [curImage originX], [curImage originY])];
	...
	[curvedMPRReferenceLineOfAxis setName: @"Axis Reference Line" ];
	[curvedMPRReferenceLineOfAxis setROIMode:ROI_sleep];
...
- (IBAction)exportCenterlines:(id)sender
{
	...
	if (parent)
	{
		...
		[temparray addObject:@"Centerlines"];
	}
	//[[self window] setFrame:screenrect display:YES ];
	[[self window] makeKeyAndOrderFront:parent];
}
...
- (void)initVesselAnalysis
{
	...
	RGBColor color;
	DCMPix * curImage= [axViewPixList objectAtIndex:0];
	axViewMeasurePolygon=[[ROI alloc] initWithType: tClosedPolygon :[curImage pixelSpacingX] :[curImage pixelSpacingY] : NSMakePoint( [curImage originX], [curImage originY])];
	...
	cViewMeasurePolygon= [[ROI alloc] initWithType: tClosedPolygon :[curImage pixelSpacingX] :[curImage pixelSpacingY] : NSMakePoint( [curImage originX], [curImage originY])];
...
- (void) creatAxROIListFromFuzzyConnectedness:(NSMutableArray*) roiList :(int) width:(int)height:(float *)im:(float)spaceX:(float)spaceY:(float)originX:(float)originY
{
    ...
    if (connectednessROIBuffer &&
        connectednessROIBufferMaxSize < (signed)(axViewCostMapHeight*axViewCostMapWidth*sizeof(char)))
	{
		free(connectednessROIBuffer);
...
-(int) thresholdLeveLSetAlgorithm:(float*)imgdata
                                 :(int)imgwidth
                                 :(int)imgheight
                                 :(int)offsetx
                                 :(int)offsety
                                 ...
                                 :(int)seedmode
{
    ...
    int pointnumber = output->GetNumberOfPoints();
	if (pointnumber<3)
		return 1;
...
- (IBAction)setAxViewThreshold:(id)sender
{
	...
	if (axViewLowerThresholdFloat == [axViewLowerThresholdText floatValue] &&
...
- (IBAction)refineCenterlineWithLongitudeSection:(id)sender
{
	...
	free(outputcenterlinexyz);
    free(outputcenterlinenormals);
    free(unitribbonxyz);
	[self performLongitudeSectionAnalysis];
	[self vesselAnalysisSetNewSource:self];
	[cViewMeasurePolygon setComments: @"done"];
...
-(float)findingCenterOfSegment:(unsigned char*)buffer
                              :(int)width
                              :(int)height
                              :(int*)center
{
...
-(void)findingGravityCenterOfSegment:(unsigned char*)buffer
                                    :(int)width
                                    :(int)height
                                    :(int*)center
{
...
- (int) showPolygonMeasurementPanel:(ViewerController *) vc
                                   :(CMIV_CTA_TOOLS*) owner
{
	originalViewController=vc;	
	parent = owner;
...
- (void) creatPolygonROIsMeasurementROIsForViewController
{
	...
	for (unsigned int i=0; i<[roiList count]; i++)
		[self creatPolygonROIsMeasurementROIsForAImage:[roiList objectAtIndex:i]
                                                      :[pixList objectAtIndex:i]
                                                      :needAddMin
                                                      :needAddMax
                                                      :needAddMean];
}

-(void)creatPolygonROIsMeasurementROIsForAImage:(NSMutableArray*)imgroilist
                                               :(DCMPix*) curImage
                                               :(BOOL)addMin
                                               :(BOOL)addMax
                                               :(BOOL)addMean
{
	...
	for(ii=0; ii<[imgroilist count]; ii++)
	{
		...
		if ([temproi type]==tClosedPolygon)
		{
			...
			ROI* minDiameterROI=[[ROI alloc] initWithType: tMeasure :[curImage pixelSpacingX] :[curImage pixelSpacingY] : NSMakePoint( [curImage originX], [curImage originY])];
			...
			ROI* maxDiameterROI=[[ROI alloc] initWithType: tMeasure :[curImage pixelSpacingX] :[curImage pixelSpacingY] : NSMakePoint( [curImage originX], [curImage originY])];
...
- (void) runSegmentation
{
	if (!contrastVolumeData)
	{
		NSRunAlertPanel(NSLocalizedString(@"no seed found", nil),
                        NSLocalizedString(@"You have to plant seeds first, choose the label from the left list and draw on the left lower view", nil),
                        NSLocalizedString(@"OK", nil), nil, nil);
		return;
	}
...
-(void)mergeVesselnessAndIntensityMap:(float*)img
                                     :(float*)vesselimg
                                     :(int)size
{
